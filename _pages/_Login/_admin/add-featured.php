			...
			<div id='main-back-end'>
				...
				<div id='child-main'>
					...
					<table border='1'  cellpadding='5' class='c-stripe-tr'>
						...
						<?php
							$query_data = mysql_query("SELECT p.id as id_brg,pd.name as nama_brg,pf.status as status FROM product p,product_detail pd,product_featured pf where pd.id_product=p.id AND pf.id_product=p.id AND pf.status='ya'");
							...
							if ($cek==0) {
								...
								while ($data_featured = mysql_fetch_array($query_data)) {
									echo "
										<tr>
											<td>".$no."</td>
											<td>$data_featured[id_brg]</td>
											<td>$data_featured[nama_brg]</td>
											<td>Featured</td>
											<td><a href='../_plugin/_config/proses.php?p=hapus-featured&id-brg=$data_featured[id_brg]'><button id='delete-button'><p>Hapus</p></button></a></td>
										</tr>
									";
									$no++;
								}
						...
						<tr id='collapse1' style='display:none;'>
							<form method='post' action='../_plugin/_config/proses.php?p=add-featured'>
								<td colspan='2'>
									<select name='nama-brg' id='combo-normal'>
										<?php
											$brgnya = mysql_query("SELECT * FROM product p,product_detail pd,product_featured pf where p.id=pd.id_product AND p.id=pf.id_product and pf.status!='ya'");
								...
								<td>
									<input type='radio' name='fea-o-not' value='ya'>Featured
								</td>
								<td width='50px'>
